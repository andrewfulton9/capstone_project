...
class GoogleImageExtractor(object):
    ...
    def __init__(self, search_key=''):
        ...
        self.folder_main_dir_prefix = 'images'
    ...
    def extract_pic_url(self):
        ...
        for tag in tag_list[:self.image_dl_per_search]:
            tag_str = re.search('imgurl=(.*)&imgrefurl', tag.attributes['href'])
            try:
    ...
    def download_single_image(self, url_link, pic_prefix_str):
        ...
        temp_filename_full_path = os.path.join(self.gs_raw_dirpath,
                                               temp_filename)
        valid_image_ext_list = ['.png', '.jpg', '.jpeg',
                                '.gif', '.bmp', '.tiff']
        url = URL(url_link)
        print url
        if url.redirect:
